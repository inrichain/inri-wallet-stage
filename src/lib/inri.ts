...
export async function resolveTokenMetadata(
  ...
  const contract = new ethers.Contract(cleanAddress, ERC20_ABI, getProvider(networkKey));
  ...
  return {
    ...
    symbol: String(symbol || "TOKEN").trim().toUpperCase(),
    decimals: Number(decimals),
    logo: guessTokenLogo(String(symbol || "")),
  };
}
