...
export default function BridgeScreen({ theme, lang, address, activeNetworkId = DEFAULT_NETWORK_ID }: { theme: "dark" | "light"; lang: string; address: string; activeNetworkId?: string; }) {
  ...
  const tokens = getDefaultTokens(fromNetwork.id).filter((tkn) => ["iUSD", "DNR", fromNetwork.nativeCurrency.symbol].includes(tkn.symbol));
  ...
  return <div style={{ display: "grid", gap: 16 }}>
    <section style={panel(isLight)}><div style={title(isLight)}>{t.bridge}</div><div style={sub(isLight)}>{t.subtitle}</div></section>
    <section style={panel(isLight)}>
      <div style={{ display: "grid", gap: 12 }}>
        ...
        <div><div style={label(isLight)}>{t.to}</div><select value={toNetworkId} onChange={(e) => setToNetworkId(e.target.value)} style={input(isLight)}>{targets.map((network) => <option key={network.id} value={network.id}>{network.name}</option>)}</select></div>
        <div><div style={label(isLight)}>{t.asset}</div><select value={tokenIndex} onChange={(e) => setTokenIndex(Number(e.target.value))} style={input(isLight)}>{tokens.map((item, idx) => <option key={idx} value={idx}>{item.symbol}</option>)}</select></div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}><img src={token?.logo} alt={token?.symbol} style={{ width: 36, height: 36, objectFit: "contain", borderRadius: 12, background: isLight ? "#f8fbff" : "#0c1422", padding: 5 }} /><div style={{ color: isLight ? "#09111f" : "#fff", fontWeight: 900 }}>{token?.symbol} • {token?.name}</div></div>
        <div><div style={label(isLight)}>{t.amount}</div><input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" style={input(isLight)} /></div>
      </div>
    </section>
    <section style={panel(isLight)}>
