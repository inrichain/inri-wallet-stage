...
export default function BridgeScreen({
  ...
  const [direction, setDirection] = useState<"polygon_to_inri" | "inri_to_polygon">(
    "polygon_to_inri"
  );
  ...
  return (
    <div style={wrap(isLight)}>
      ...
      <div style={banner(isLight)}>
        {BRIDGE_CONFIGURED ? t.bridgeReady : t.bridgePending}
      </div>
      ...
      <div style={{ ...panel(isLight), marginTop: 12 }}>
        ...
        <div style={doubleGrid}>
          <button
            onClick={() => setDirection("polygon_to_inri")}
            style={modeButton(isLight, direction === "polygon_to_inri")}
          >
            {t.depositFlow}
          </button>
          <button
            onClick={() => setDirection("inri_to_polygon")}
            style={modeButton(isLight, direction === "inri_to_polygon")}
          >
            {t.withdrawFlow}
          </button>
        </div>
      ...
      <div style={{ ...panel(isLight), marginTop: 12 }}>
        ...
        <div style={estimatedStyle(isLight)}>
          {estimated} {toToken.symbol}
        </div>

        <div style={miniRow(isLight)}>
          <span>{t.route}</span>
          <strong>{fromToken.network} → {toToken.network}</strong>
        </div>

        <div style={miniRow(isLight)}>
          <span>{t.assetFlow}</span>
          <strong>{fromToken.symbol} → {toToken.symbol}</strong>
        </div>
        ...
        <div style={miniRow(isLight)}>
          <span>{t.status}</span>
          <strong>{BRIDGE_CONFIGURED ? t.live : t.awaitingContracts}</strong>
        </div>
      </div>
      ...
      <div style={{ ...panel(isLight), marginTop: 12 }}>
        ...
        <div style={slotsGrid}>
          <LogoSlot
            ...
            subtitle={t.slotHint}
            isLight={isLight}
            image={BASE + "token-usdt.png"}
          />
          <LogoSlot
            title="INRI / iUSD"
            subtitle={t.slotReady}
            isLight={isLight}
      ...
      <button
        style={{ ...mainButtonStyle(), opacity: BRIDGE_CONFIGURED ? 1 : 0.7 }}
        disabled={!BRIDGE_CONFIGURED}
      >
        {BRIDGE_CONFIGURED ? t.bridgeNow : t.waitingConfig}
      </button>
    </div>
  );
}
...
function BridgeAssetCard({
  ...
  return (
    <div style={{ textAlign: align, minWidth: 0 }}>
      ...
      >
        {align === "right" ? null : (
          <img src={asset.logo} alt={asset.symbol} style={tokenLogo} />
        )}
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <img
          src={image || "/token-placeholder.svg"}
          alt={title}
...
function title(isLight: boolean): React.CSSProperties {
  return { margin: "0", color: isLight ? "#10131a" : "#ffffff" };
}
...
function banner(isLight: boolean): React.CSSProperties {
  return {
    marginBottom: 12,
    padding: "12px 14px",
    borderRadius: 14,
...
function routeCard(isLight: boolean): React.CSSProperties {
  return {
    display: "grid",
...
function routeLabel(isLight: boolean): React.CSSProperties {
  return { fontSize: 12, color: isLight ? "#64748b" : "#94a3b8", fontWeight: 700 };
}
function routeSub(isLight: boolean): React.CSSProperties {
  return {
    fontSize: 12,
...
const tokenLogo: React.CSSProperties = {
  width: 42,
  height: 42,
  borderRadius: 14,
  objectFit: "cover",
...
function miniRow(isLight: boolean): React.CSSProperties {
  return {
    display: "flex",
    justifyContent: "space-between",
    gap: 12,
...
function modeButton(isLight: boolean, active: boolean): React.CSSProperties {
  return {
    padding: "13px 14px",
    borderRadius: 14,
    border: active ? "1px solid #4d7ef2" : `1px solid ${isLight ? "#dbe2f0" : "#252b39"}`,
    background: active ? "#3f7cff" : isLight ? "#ffffff" : "#12192a",
    color: active ? "#ffffff" : isLight ? "#10131a" : "#fff",
    fontWeight: 800,
    cursor: "pointer",
  };
}
...
function hero(isLight: boolean): React.CSSProperties {
  return {
    marginTop: 12,
    padding: 16,
    borderRadius: 20,
    ...
    background: isLight
      ...
      : "linear-gradient(180deg,#12192a 0%, #0f1522 100%)",
    display: "grid",
    gridTemplateColumns: "1fr auto 1fr",
    gap: 16,
    alignItems: "center",
...
const heroSide: React.CSSProperties = {
  display: "flex",
  alignItems: "center",
  gap: 12,
  minWidth: 0,
};
const heroTokenLogo: React.CSSProperties = {
  width: 54,
  height: 54,
  borderRadius: 18,
  objectFit: "cover",
...
function heroTokenTitle(isLight: boolean): React.CSSProperties {
  return { fontSize: 20, fontWeight: 900, color: isLight ? "#10131a" : "#ffffff" };
}
function heroTokenSub(isLight: boolean): React.CSSProperties {
