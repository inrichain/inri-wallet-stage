...
import { TokenItem, fetchTokenMetadata, getDefaultTokensForNetwork, loadAllBalances } from "../lib/inri";
...
export default function TokensScreen({
  ...
  useEffect(() => {
    ...
    if (!/^0x[a-fA-F0-9]{40}$/.test(cleanAddress)) return;

    let active = true;
    const timer = setTimeout(async () => {
      try {
        setIsLoadingTokenData(true);
        const meta = await fetchTokenMetadata(cleanAddress, networkKey);
        if (!active) return;
        if (meta.symbol && !symbol.trim()) setSymbol(meta.symbol.toUpperCase());
        if (Number.isFinite(meta.decimals)) setDecimals(String(meta.decimals));
        showMessage(t.tokenDataLoaded);
      } catch {
        if (!active) return;
        showMessage(t.tokenDataNotFound);
      } finally {
        if (active) setIsLoadingTokenData(false);
      }
    }, 450);
    ...
    >
      ...
      <div style={{ display: "grid", gap: 10, marginBottom: 16 }}>
        ...
        <button onClick={addToken} style={mainButtonStyle()}>
          {isLoadingTokenData ? t.loadingTokenData : t.addToken}
        </button>
...
function getText(lang: string) {
  const map: Record<string, any> = {
    en: {
      ...
      addToken: "Add Token",
      loadingTokenData: "Loading token...",
      ...
      tokenDataLoaded: "Token data loaded automatically.",
      tokenDataNotFound: "Could not read this token automatically.",
      fixed: "Fixed",
      remove: "Remove",
    },
    pt: {
      ...
      loadingTokenData: "Buscando token...",
      ...
      tokenDataLoaded: "Dados do token carregados automaticamente.",
      tokenDataNotFound: "Não foi possível ler este token automaticamente.",
      fixed: "Fixo",
      remove: "Remover",
    },
    es: {
      ...
      tokenDataLoaded: "Datos del token cargados automáticamente.",
      tokenDataNotFound: "No fue posible leer este token automáticamente.",
      fixed: "Fijo",
      remove: "Eliminar",
    },
  };
