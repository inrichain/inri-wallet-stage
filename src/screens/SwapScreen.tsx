...
export default function SwapScreen({ theme, lang, address, activeNetworkId = DEFAULT_NETWORK_ID }: { theme: "dark" | "light"; lang: string; address: string; activeNetworkId?: string; }) {
  ...
  const [toIndex, setToIndex] = useState(Math.min(1, Math.max(tokens.length - 1, 0)));
  ...
  useEffect(() => {
    if (toIndex === fromIndex && tokens.length > 1) setToIndex((fromIndex + 1) % tokens.length);
  }, [fromIndex, toIndex, tokens.length]);
  ...
  useEffect(() => {
    ...
    async function loadBalance() {
      ...
      try {
        ...
        if (!mounted) return;
        setBalanceIn(formatTokenAmount(value));
        setRpcLabel(value ? t.ready : t.ready);
      } catch {
  ...
  }, [address, fromToken, network, t.ready]);
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <section style={panel(isLight)}>
        <div style={title(isLight)}>{t.swap}</div>
        <div style={sub(isLight)}>{network.name} • {rpcLabel}</div>
      </section>
      ...
      <section style={panel(isLight)}>
        ...
        <div style={{ display: "flex", justifyContent: "center", margin: "12px 0" }}>
          <button onClick={flip} style={flipBtn(isLight)}>⇅</button>
        </div>
      ...
      <section style={panel(isLight)}>
        ...
        <Row isLight={isLight} label={t.minimumReceived} value={`${previewOut} ${toToken?.symbol || ""}`} />
...
function TokenCard({ isLight, label, token, amount, setAmount, tokenIndex, setTokenIndex, tokens, balance, readOnly }: any) {
  return (
    <div style={{ border: `1px solid ${isLight ? "#dbe3f2" : "#22314f"}`, borderRadius: 22, padding: 16, background: isLight ? "#f8fbff" : "#0c1422" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
        <div style={{ color: isLight ? "#64748f" : "#90a3c7", fontWeight: 800, fontSize: 12 }}>{label}</div>
        <div style={{ color: isLight ? "#64748f" : "#90a3c7", fontWeight: 800, fontSize: 12 }}>Balance: {balance}</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 170px", gap: 12, marginTop: 12 }}>
        <input value={amount} onChange={(e) => setAmount(e.target.value)} readOnly={readOnly} placeholder="0.00" style={{ width: "100%", padding: 14, borderRadius: 16, border: `1px solid ${isLight ? "#dbe3f2" : "#22314f"}`, background: isLight ? "#fff" : "#101827", color: isLight ? "#09111f" : "#fff", fontWeight: 900, fontSize: 18, boxSizing: "border-box" }} />
        <select value={tokenIndex} onChange={(e) => setTokenIndex(Number(e.target.value))} style={{ width: "100%", padding: 12, borderRadius: 16, border: `1px solid ${isLight ? "#dbe3f2" : "#22314f"}`, background: isLight ? "#fff" : "#101827", color: isLight ? "#09111f" : "#fff" }}>
          {tokens.map((item: any, idx: number) => <option key={idx} value={idx}>{item.symbol}</option>)}
        </select>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
        <img src={token?.logo} alt={token?.symbol} style={{ width: 40, height: 40, objectFit: "contain", borderRadius: 12, background: isLight ? "#fff" : "#101827", padding: 5 }} />
        <div>
          <div style={{ color: isLight ? "#0f172a" : "#fff", fontWeight: 900 }}>{token?.symbol}</div>
          <div style={{ color: isLight ? "#64748f" : "#90a3c7", fontSize: 12 }}>{token?.name}</div>
        </div>
      </div>
    </div>
  );
...
function getText(lang: string) { const map: Record<string, any> = { en: { swap: "Swap", from: "From", to: "To", route: "Route", networkFee: "Swap fee", priceImpact: "Price impact", minimumReceived: "Minimum received", comingSoon: "Swap execution coming soon", ready: "RPC ready" }, pt: { swap: "Swap", from: "De", to: "Para", route: "Rota", networkFee: "Taxa do swap", priceImpact: "Impacto no preço", minimumReceived: "Mínimo recebido", comingSoon: "Execução do swap em breve", ready: "RPC pronto" } }; return map[lang] || map.en; }
