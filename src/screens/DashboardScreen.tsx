  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <section style={card}>
        ...
        <div style={{ marginTop: 10, display: "inline-flex", padding: "8px 12px", borderRadius: 999, background: isLight ? "#eef4ff" : "#16213b", color: "#3f7cff", fontWeight: 800, fontSize: 13 }}>{network.name} • Chain {network.chainId}</div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
          ...
          <button onClick={() => setTab("settings")} style={actionGhost(isLight)}>{t.changeNetwork}</button>
...
function actionGhost(isLight: boolean): React.CSSProperties { return { padding: "10px 14px", borderRadius: 12, border: `1px solid ${isLight ? "#dbe2f0" : "#252b39"}`, background: isLight ? "#fff" : "#121621", color: isLight ? "#10131a" : "#fff", fontWeight: 800, cursor: "pointer" }; }
function getText(lang: string) {
  const map: Record<string, any> = {
    en: { totalBalance: "Total balance", send: "Send", receive: "Receive", swap: "Swap", bridge: "Bridge", sendDesc: "Native and ERC-20 transfers", receiveDesc: "QR code and wallet address", swapDesc: "Professional swap interface with token/network selectors", bridgeDesc: "Polygon USDT ↔ INRI iUSD bridge flow", openExplorer: "Open in Explorer", installApp: "Install App", changeNetwork: "Change network" },
    pt: { totalBalance: "Saldo total", send: "Enviar", receive: "Receber", swap: "Swap", bridge: "Bridge", sendDesc: "Transferências nativas e ERC-20", receiveDesc: "QR code e endereço da carteira", swapDesc: "Interface profissional com seletor de token/rede", bridgeDesc: "Fluxo Polygon USDT ↔ INRI iUSD", openExplorer: "Abrir no Explorer", installApp: "Instalar App", changeNetwork: "Trocar rede" },
    es: { totalBalance: "Saldo total", send: "Enviar", receive: "Recibir", swap: "Swap", bridge: "Bridge", sendDesc: "Transferencias nativas y ERC-20", receiveDesc: "Código QR y dirección", swapDesc: "Interfaz profesional con selector de red/token", bridgeDesc: "Flujo Polygon USDT ↔ INRI iUSD", openExplorer: "Abrir en Explorer", installApp: "Instalar app", changeNetwork: "Cambiar red" },
  };
