...
export default function SettingsScreen({
  ...
  const wcSectionRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    if (localStorage.getItem("wallet_open_wc") === "1") {
      localStorage.removeItem("wallet_open_wc");
      window.setTimeout(() => {
        wcSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 120);
    }
  }, []);
    ...
    <>
      <div style={{ display: "grid", gap: 18 }}>
        ...
        <div style={cardStyle(isLight)}>
          ...
          <div style={{ color: isLight ? "#5b6578" : "#97a0b3", marginBottom: 14, lineHeight: 1.55 }}>
            Quick access to WalletConnect and your active sessions.
          </div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button onClick={() => wcSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })} style={mainButtonStyle()}>Open WalletConnect</button>
            <button onClick={() => networkSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })} style={secondaryButtonStyle(isLight)}>Open networks</button>
          </div>
          <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
            <div style={{ color: isLight ? "#5b6578" : "#97a0b3", fontSize: 13 }}>Active sessions: <strong style={{ color: isLight ? "#10131a" : "#ffffff" }}>{wcSessions.length}</strong></div>
            {wcSessions.slice(0, 2).map((session) => (
              <div key={session.topic} style={{ border: `1px solid ${isLight ? "#dbe2f0" : "#252b39"}`, borderRadius: 14, padding: 12, background: isLight ? "#f8faff" : "#0f1420" }}>
                <div style={{ fontWeight: 800, color: isLight ? "#10131a" : "#ffffff" }}>{session.name}</div>
                <div style={{ fontSize: 12, color: isLight ? "#5b6578" : "#97a0b3", wordBreak: "break-word", marginTop: 4 }}>{session.url || t.wcNoUrl}</div>
              </div>
            ))}
          </div>
        </div>

        <div ref={networkSectionRef} style={cardStyle(isLight)}>
          <div
            style={{
              fontWeight: 800,
        ...
        <div ref={wcSectionRef} style={cardStyle(isLight)}>
          <div
            style={{
              fontWeight: 800,
              color: isLight ? "#10131a" : "#ffffff",
