    ...
    <div style={{ display: "grid", gap: 18 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? (isLight ? "#eef4ff" : "#16213b") : (isLight ? "#ffffff" : "#12192a"),
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <img src={item.logo} alt={item.name} style={{ width: 36, height: 36, borderRadius: 12, objectFit: "cover" }} />
                  <div>
                    <div style={{ fontWeight: 800 }}>{item.name}</div>
                    <div style={{ fontSize: 12, color: isLight ? "#5b6578" : "#97a0b3" }}>Chain ID {item.chainId} • {item.symbol}</div>
                  </div>
                </div>
                <div style={{ fontWeight: 800, color: active ? "#3f7cff" : isLight ? "#64748b" : "#97a0b3" }}>{active ? t.active : t.select}</div>
              </button>
            );
          })}
        </div>
      ...
      <section style={cardStyle(isLight)}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <img src={avatar} alt="Avatar" onError={(e) => { (e.currentTarget as HTMLImageElement).src = DEFAULT_AVATAR; }} style={{ width: 76, height: 76, borderRadius: "50%", objectFit: "cover", border: `2px solid ${isLight ? "#dbe2f0" : "#2b3650"}`, background: isLight ? "#f8fafc" : "#0f172a" }} />
...
function getText(lang: string) {
  const map: Record<string, any> = {
    en: { settings: "Settings", subtitle: "Language, theme, network and avatar", language: "Language", theme: "Theme", dark: "Dark Mode", light: "Light Mode", network: "Active network", networkHint: "Network switching stays here in one place. Swap and bridge screens use the active network display only.", saveRpc: "Save RPC", avatar: "Avatar", avatarHint: "Your avatar is fully personal now. No INRI logo is forced here anymore.", changeAvatar: "Upload Avatar", removeAvatar: "Remove Avatar", active: "Active", select: "Select" },
    pt: { settings: "Configurações", subtitle: "Idioma, tema, rede e avatar", language: "Idioma", theme: "Tema", dark: "Modo Escuro", light: "Modo Claro", network: "Rede ativa", networkHint: "A troca de rede fica aqui em um só lugar. As telas de swap e bridge usam apenas a rede ativa exibida.", saveRpc: "Salvar RPC", avatar: "Avatar", avatarHint: "Seu avatar agora é totalmente pessoal. A logo da INRI não fica mais forçada aqui.", changeAvatar: "Enviar Avatar", removeAvatar: "Remover Avatar", active: "Ativa", select: "Selecionar" },
    es: { settings: "Configuración", subtitle: "Idioma, tema, red y avatar", language: "Idioma", theme: "Tema", dark: "Modo oscuro", light: "Modo claro", network: "Red activa", networkHint: "El cambio de red queda aquí en un solo lugar. Las pantallas swap y bridge usan solo la red activa mostrada.", saveRpc: "Guardar RPC", avatar: "Avatar", avatarHint: "Tu avatar ahora es totalmente personal. El logo de INRI ya no queda forzado aquí.", changeAvatar: "Subir Avatar", removeAvatar: "Eliminar Avatar", active: "Activa", select: "Seleccionar" },
  };
